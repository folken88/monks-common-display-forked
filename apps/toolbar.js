...
export class CommonToolbar extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static PARTS = {
        main: {
            root: true,
            template: "modules/monks-common-display-forked/templates/toolbar.html",
        }
    };
    ...
    onPersistPosition(position) {
        game.user.setFlag("monks-common-display-forked", "position", { left: position.left, top: position.top });
    }
    ...
    async _prepareContext(options) {
        ...
        let screen = (setting("per-scene") ? foundry.utils.getProperty(canvas.scene, "flags.monks-common-display-forked.screen") : setting("screen")) || "gm";
        let focus = (setting("per-scene") ? foundry.utils.getProperty(canvas.scene, "flags.monks-common-display-forked.focus") : setting("focus")) || "gm";
    ...
    getPos() {
        this.pos = game.user.getFlag("monks-common-display-forked", "position");
        ...
        if (this.pos == undefined) {
            ...
            game.user.setFlag("monks-common-display-forked", "position", this.pos);
        }
    ...
    static async toggleScreen() {
        if (!!MonksCommonDisplay.selectToken) {
            ...
            if (setting("per-scene")) {
                await canvas.scene.setFlag("monks-common-display-forked", MonksCommonDisplay.selectToken, tokenids);
                foundry.utils.setProperty(canvas.scene, `flags.monks-common-display-forked.${MonksCommonDisplay.selectToken}`, tokenids);
            } else {
                await game.settings.set("monks-common-display-forked", MonksCommonDisplay.selectToken, tokenids);
            }
            ...
            await game.settings.set("monks-common-display-forked", "screen-toggle", active);
            if (active) {
    ...
    static async toggleFocus() {
        ...
        await game.settings.set("monks-common-display-forked", "focus-toggle", active);
        ...
        return [
            {
                ...
                callback: async (btn) => {
                    ...
                    MonksCommonDisplay.selectToken = null;
                    if (setting("per-scene"))
                        await canvas.scene.setFlag("monks-common-display-forked", group, "gm");
                    else
                        await game.settings.set("monks-common-display-forked", group, "gm");
            ...
            },
            {
                ...
                callback: async (btn) => {
                    ...
                    MonksCommonDisplay.selectToken = null;
                    if (setting("per-scene"))
                        await canvas.scene.setFlag("monks-common-display-forked", group, "controlled");
                    else
                        await game.settings.set("monks-common-display-forked", group, "controlled");
                    if (group == "screen") MonksCommonDisplay.screenChanged(); else MonksCommonDisplay.focusChanged();
                    this.render(true);
                }
            },
            {
                ...
                callback: async (btn) => {
                    ...
                    MonksCommonDisplay.selectToken = null;
                    if (setting("per-scene"))
                        await canvas.scene.setFlag("monks-common-display-forked", group, "scene");
                    else
                        await game.settings.set("monks-common-display-forked", group, "scene");
                    if (group == "screen") MonksCommonDisplay.screenChanged(); else MonksCommonDisplay.focusChanged();
                    this.render(true);
                }
            },
            {
                ...
                callback: async (btn) => {
                    ...
                    MonksCommonDisplay.selectToken = null;
                    if (setting("per-scene"))
                        await canvas.scene.setFlag("monks-common-display-forked", group, "combat");
                    else
                        await game.settings.set("monks-common-display-forked", group, "combat");
                    if (group == "screen") MonksCommonDisplay.screenChanged(); else MonksCommonDisplay.focusChanged();
                    this.render(true);
                }
            },
            {
                ...
                callback: async (btn) => {
                    ...
                    MonksCommonDisplay.selectToken = null;
                    if (setting("per-scene"))
                        await canvas.scene.setFlag("monks-common-display-forked", group, "party");
                    else
                        await game.settings.set("monks-common-display-forked", group, "party");
                    if (group == "screen") MonksCommonDisplay.screenChanged(); else MonksCommonDisplay.focusChanged();
                    this.render(true);
                }
            },
