...
export let debug = (...args) => {
    if (DEBUG) console.log("DEBUG: monks-common-display-forked | ", ...args);
};
export let log = (...args) => console.log("monks-common-display-forked | ", ...args);
export let warn = (...args) => console.warn("monks-common-display-forked | ", ...args);
export let error = (...args) => console.error("monks-common-display-forked | ", ...args);
export let i18n = key => {
    return game.i18n.localize(key);
};
export let setting = key => {
    return game.settings.get("monks-common-display-forked", key);
};
...
export let patchFunc = (prop, func, type = "WRAPPER") => {
    ...
    if (game.modules.get("lib-wrapper")?.active) {
        try {
            libWrapper.register("monks-common-display-forked", prop, func, type);
        } catch (e) {
            nonLibWrapper();
        }
    } else {
...
export class MonksCommonDisplay {
    ...
    static init() {
        MonksCommonDisplay.SOCKET = "module.monks-common-display-forked";

        registerSettings();
        MonksCommonDisplay.registerHotKeys();

        if (game.modules.get("lib-wrapper")?.active) {
            libWrapper.ignore_conflicts("monks-common-display-forked", "monks-active-tiles", "foundry.applications.sidebar.tabs.ActorDirectory.prototype._onClickEntryName");
        }
        ...
        patchFunc("foundry.applications.sidebar.tabs.ActorDirectory.prototype._onClickEntry", async function (wrapped, ...args) {
            ...
            if (!!MonksCommonDisplay.selectToken) {
                ...
                if (setting("per-scene")) {
                    await canvas.scene.setFlag("monks-common-display-forked", MonksCommonDisplay.selectToken, documentId);
                    foundry.utils.setProperty(canvas.scene, `flags.monks-common-display-forked.${MonksCommonDisplay.selectToken}`, documentId);
                } else {
                    await game.settings.set("monks-common-display-forked", MonksCommonDisplay.selectToken, documentId);
                }
    ...
    static async ready() {
        ...
        if (setting("show-toolbar") && game.user.isGM) {
            let {top, left } = game.user.getFlag("monks-common-display-forked", "position");
    ...
    static dataChange() {
        ...
        game.settings.set('monks-common-display-forked', 'startupdata', MonksCommonDisplay.playerdata.display);
    ...
    static registerHotKeys() {
        game.keybindings.register('monks-common-display-forked', 'clear-images', {
        ...
        game.keybindings.register('monks-common-display-forked', 'clear-journals', {
    ...
    /**
     * Update the common display combat portrait for the active combatant.
     * @returns {void}
     */
    static updateCombatPortrait() {
        if (!game || !game.combats) {
            return;
        }
        const isDisplay = MonksCommonDisplay.playerdata.display || false;
        const hasCombat = game.combats.active && game.combats.active.started;
        const showPortrait = setting("show-combat-portrait");
        const shouldShow = isDisplay && setting("show-combat") && showPortrait && hasCombat;

        const sizeSetting = setting("combat-portrait-size") || "large";
        const sizeMap = {
            small: "clamp(160px, 16vw, 260px)",
            medium: "clamp(220px, 22vw, 360px)",
            large: "clamp(260px, 28vw, 520px)"
        };
        const portraitWidth = sizeMap[sizeSetting] || sizeMap.large;
        const opacitySetting = setting("combat-portrait-opacity");
        const opacityValue = Number.isFinite(opacitySetting) ? opacitySetting : 95;
        const portraitOpacity = Math.max(0, Math.min(100, opacityValue)) / 100;

        document.documentElement.style.setProperty("--mcd-combat-portrait-width", portraitWidth);
        document.documentElement.style.setProperty("--mcd-combat-portrait-opacity", portraitOpacity.toString());

        if (document.body) {
            document.body.classList.toggle("mcd-show-combat-portrait", shouldShow);
        }

        let portrait = document.getElementById("mcd-combat-portrait");
        if (!shouldShow) {
        ...
        const imgSrc =
            ...
            combatant?.actor?.prototypeToken?.img ||
            combatant?.img ||
        ...
        if (!portrait) {
            ...
            portrait.id = "mcd-combat-portrait";
            portrait.className = "mcd-combat-portrait";
            const img = document.createElement("img");
            img.className = "mcd-combat-portrait__image";
            portrait.appendChild(img);
            document.body.appendChild(portrait);
        }

        const imgEl = portrait.querySelector("img");
        if (imgEl) {
            imgEl.src = imgSrc;
    ...
    static get screenValue() {
        return setting("per-scene") ? foundry.utils.getProperty(canvas.scene, "flags.monks-common-display-forked.screen") : setting("screen");
    }

    static get focusValue() {
        return setting("per-scene") ? foundry.utils.getProperty(canvas.scene, "flags.monks-common-display-forked.focus") : setting("focus");
    }
...
Hooks.on('renderSceneControls', async (control, html, data) => {
    if (game.user.isGM && $('#scene-controls-layers .common-display', html).length == 0) {
        ...
        btn.on('click', async () => {
            ...
            game.settings.set('monks-common-display-forked', 'show-toolbar', toggled);
            if (toggled) {
...
Hooks.on("getUserContextOptions", (app, menuitems) => {
    menuitems.push({
        ...
        callback: li => {
            ...
            game.settings.set('monks-common-display-forked', 'playerdata', playerdata).then(() => {
